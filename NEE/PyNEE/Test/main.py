...
"""
 ...
 # Create dataset
RNN.nb_of_samples = 20
sequence_len = 10
# Create the sequences
X = np.zeros((RNN.nb_of_samples, sequence_len))
for row_idx in range(RNN.nb_of_samples):
...
S = RNN.forward_states(X, params[0], params[1])
grad_out = RNN.output_gradient(S[:,-1], t)
backprop_grads, grad_over_time = RNN.backward_gradient(X, S, grad_out, params[1])
...
for p_idx, _ in enumerate(params):
    ...
    # + eps
    params[p_idx] += eps
    plus_cost = RNN.cost(RNN.forward_states(X, params[0], params[1])[:,-1], t)
    # - eps
    params[p_idx] -= 2 * eps
    min_cost = RNN.cost(RNN.forward_states(X, params[0], params[1])[:,-1], t)
    # reset param value
    params[p_idx] += eps
...
def get_grad_over_time(wx, wRec):
    ...
    S = RNN.forward_states(X, wx, wRec)
    grad_out = RNN.output_gradient(S[:,-1], t).sum()
    _, grad_over_time = RNN.backward_gradient(X, S, grad_out, wRec)
...
fig = get_cost_surface_figure(lambda w1, w2: RNN.cost(RNN.forward_states(X, w1, w2)[:,-1] , t), points)
